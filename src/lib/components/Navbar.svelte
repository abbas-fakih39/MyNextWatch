	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex items-center justify-between h-16 gap-4">
			...
			<div class="hidden md:flex md:items-center md:gap-6">
				<a href="/" class="text-muted hover:text-text transition-colors text-sm font-medium">Accueil</a>
			...
			<div class="flex items-center gap-3 shrink-0">
				...
				<!-- Mobile menu button -->
				<button onclick={toggleMenu} class="md:hidden text-muted hover:text-text focus:outline-none">
					{#if isMobileMenuOpen}
						<X class="h-6 w-6" />
					{:else}
						<Menu class="h-6 w-6" />
	...
	{#if isMobileMenuOpen}
		<div class="md:hidden bg-surface border-b border-border">
			...
			<div class="px-2 pb-3 space-y-1">
				<a href="/" class="block px-3 py-2 rounded-md text-sm font-medium text-text hover:bg-border" onclick={toggleMenu}>
					Accueil
				</a>
