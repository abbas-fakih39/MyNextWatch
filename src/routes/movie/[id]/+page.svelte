<script lang="ts">
	...
	type WatchlistStatus = 'plan_to_watch' | 'watching' | 'completed' | 'on_hold' | 'dropped';
	...
	function setStatus(status: WatchlistStatus) {
		watchlistStatus = status;
		watchlistOpen = false;
		// TODO: persist to Supabase watchlist table when backend is ready
	}
		...
		<div class="max-w-400 mx-auto px-4 sm:px-6 lg:px-8 -mt-40 relative z-10">
				...
				<div class="flex-1 pt-2 sm:pt-24">
					...
					<div class="relative inline-block">
						<button
							...
							class="inline-flex items-center gap-2 px-5 py-2.5 rounded-md font-semibold text-sm transition-colors border {watchlistStatus
								? 'bg-accent text-bg border-accent'
								: 'bg-surface border-border text-text hover:border-accent hover:text-accent'}"
						>
							{watchlistStatus ? statusLabels[watchlistStatus] : '+ Ajouter à ma watchlist'}
						</button>
							...
							>
								{#each statusOptions as option (option.value)}
									<button
										...
										class="w-full text-left px-4 py-2.5 text-sm hover:bg-border transition-colors {watchlistStatus === option.value ? 'text-accent' : 'text-text'}"
									>
										{option.label}
									</button>
								{/each}
			...
			{#if similarItems.length > 0}
				<div class="-mx-4 sm:-mx-6 lg:-mx-8 mt-4">
					<MediaGrid title="Films similaires" items={similarItems} limit={12} />
				</div>
			{/if}
		</div>
	</div>
